...
dword GetRewardValue(dword function, dword amount, CPlayer* client, word nDupCNT){
    int Cha = client->Attr->Cha + client->Attr->Cha;
	switch(function){
		case 0:
		{
			dword tempVal = amount + 0x1E;
            tempVal *= Cha + 0x0a; // We need to add a way to get all stats totals (Including buff, E, etc) - Drakia
		...
		case 1:
		{
			...
			tempVal *= (Cha >> 1) + client->Stats->Level + 0x28;
			tempVal *= (100 & 0xFFFF);//World Rate
		...
		case 4:
		{
			...
			tempVal *= (Cha + client->Stats->Level + 0x28);
		...
		{
			dword tempVal = amount + 0x14;
			tempVal *= Cha + client->Stats->Level;
			tempVal *= 0x14;//Fame + 0x14
			tempVal *= (100 & 0xFFFF);//World Rate
            ...
            for(dword i = 0; i < 10; i++)
            {
                ...
                if(data->iQuestSN >= 2851&&data->iQuestSN <= 2858)
                {
                    ...
                    if (zaward > 0)
                    {
                        ...
                        Log(MSG_QUESTDEBUG,"QUESTREWD(000):: Giving %u zuly to player %s",zaward,client->CharInfo->charname);
                    }
...
QUESTREWD(003)
{
	...
	for(dword i = 0; i < data->iDataCnt; i++)
	{
        ...
        case 85:
        {
            ...
            client->client->SendPacket( &pak );
            if( client->questdebug )
                Log(MSG_QUESTDEBUG, "QUESTREWD(003):: SET union0%i points adding %i (new value %i,%i,%i,%i,%i)",curAbil->iType,curAbil->iValue,client->CharInfo->union01,client->CharInfo->union02,client->CharInfo->union03,client->CharInfo->union04,client->CharInfo->union05);
        }
        break;
		case sStrength:
		...
		{
			if( client->questdebug )
                Log(MSG_QUESTDEBUG, "QUESTREWD(003):: Type Unknown: '%i'", curAbil->iType);
		}
        break;
...
QUESTREWD(005)
{
	...
	switch(data->btTarget){
		case 0://EXP
		{
			if(!client->no_exp && client->Stats->Level < GServer->Config.MaxLevel)
            {
                if(client->Position->Map == GServer->Config.doubleExpMap)
                {
                    client->CharInfo->Exp += (GetRewardValue(data->btEquation, data->iValue, client, 0) * (GServer->Config.doubleExpAmount * GServer->Config.EXP_RATE));
                }else
                {
                    client->CharInfo->Exp += (GetRewardValue(data->btEquation, data->iValue, client, 0) * GServer->Config.EXP_RATE);
                }
		...
		{
		    if( client->questdebug )
                Log(MSG_QUESTDEBUG, "QUESTREWD(005):: recieve buff. Starting attack so that AIP can apply the buffs" );
		    CCharacter* monster = GServer->GetMonsterByID(client->quest.RefNPC,client->Position->Map);
		    monster->StartAction( client, SKILL_ATTACK, nSkill );
		}
		break;
		default:
			Log(MSG_WARNING,"QUESTREWD(005):: UnHandled target type: %i",data->btTarget);
		break;
...
QUESTREWD(006)
{
	...
	if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTREWD(006):: Restoring HP and MP" );
...
QUESTREWD(007)
{
    ...
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTREWD(007):: Teleport to new location" );
...
QUESTREWD(013)
{
	...
	//This is a quest trigger with a timer delay. eg. respawn NPCs for the missing children
	GETREWDDATA(013);
	char* tempName = reinterpret_cast<char*>(&data->szTriggerName) - 2;
	dword hash = MakeStrHash(tempName);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTREWD(013):: Execute Quest Trigger %s[%d] [%08x] with time delay", tempName, data->nNameLength, hash);
    int tempval = client->quest.RefNPC;
    //int is_ok = client->ExecuteQuestTrigger(hash,true);
    client->quest.RefNPC = tempval;
    //PY the following stuff is unavailable in this server version at this time
...
//Group Flag Clear. Imported from 137. Code may not work here
QUESTREWD(016)
{
	//word nGroupSN
	GETREWDDATA(016);
	...
	//client->quest.dwFlags[data->nGroupSN] = 0;  //PY: don't think this server version has this data structure
	return QUEST_SUCCESS;
}
...
QUESTREWD(018)
{
    ...
    GServer->NPCAnnounce(tempName, npcName);
    return QUEST_SUCCESS;
}
...
QUESTREWD(030) //Reset skills
{
    ...
    //PY need to recode this for this server version. just disable it for now
    /*
    for(int i=0;i<30;i++)
    {
...
QUESTREWD(034)
{
    ...
    if (client->questdebug)
        Log(MSG_QUESTDEBUG, "QUESTREWD(034):: removing selected NPC");
