...
void CWorldServer::pakPlayer( CPlayer *thisclient )
{
    ...
    if(!thisclient->is_born)
    {
        ...
        if(hash_zone>0)
        {
            ...
            if(GServer->ServerDebug)
                Log(MSG_DEBUG,"Executing Quest Trigger pakPlayer");
    ...
    if(thisclient->items[142].itemnum != 0)
    {
        ADDWORD    ( pak, thisclient->items[142].itemnum );
        ADDWORD    ( pak, BuildItemRefine( thisclient->items[142] ) );
    }else
    {
    ...
    if(thisclient->items[143].itemnum != 0)
    {
        ADDWORD    ( pak, thisclient->items[143].itemnum );
        ADDWORD    ( pak, BuildItemRefine( thisclient->items[143] ) );
    }else
    {
    ...
    if(thisclient->items[144].itemnum != 0)
    {
        ADDWORD    ( pak, thisclient->items[144].itemnum );
        ADDWORD    ( pak, BuildItemRefine( thisclient->items[144] ) );
    }else
    {
    ...
    if(thisclient->items[145].itemnum != 0)
    {
        ADDWORD    ( pak, thisclient->items[145].itemnum );
        ADDWORD    ( pak, BuildItemRefine( thisclient->items[145] ));
    }else
    {
    ...
    if(thisclient->items[146].itemnum != 0)
    {
        ADDWORD    ( pak, thisclient->items[146].itemnum );
        ADDWORD    ( pak, BuildItemRefine( thisclient->items[146] ) );
    }else
    {
    ...
    if(thisclient->items[147].itemnum != 0)
    {
        ADDWORD    ( pak, thisclient->items[147].itemnum );
        ADDWORD    ( pak, BuildItemRefine( thisclient->items[147] ) );
    }else
    {
...
void CWorldServer::pakInventoryCostume( CPlayer *thisclient )
{
    for(unsigned j=140; j<148; j++)
    thisclient->UpdateInventoryCostume(j,0);
...
bool CWorldServer::pakDoIdentify( CPlayer *thisclient, CPacket *P )
{
    ...
    if(thisclient->storageExpanded&&(etime>=thisclient->timerexapnsion))
    {
        thisclient->storageExpanded = false;
        thisclient->timerexapnsion=0;
    }
	...
	if(thisclient->timerexapnsion>0)
    {
        rawtime=thisclient->timerexapnsion;
        timeinfo = localtime ( &rawtime );
        thisclient->storageExpanded = true;
        Log(MSG_INFO,"%s:: Storage Expansion %i/%i/%i, %i:%i:%i",thisclient->CharInfo->charname,timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday,timeinfo->tm_hour+1,timeinfo->tm_min+1,timeinfo->tm_sec+1);
    }else
    {
        thisclient->storageExpanded = false;
    }
...
bool CWorldServer::pakDoID( CPlayer* thisclient, CPacket* P )
{
    ...
    if(hash_zone>0)
    {
        ...
        //int success=thisclient->ExecuteQuestTrigger(hash_zone,false);
        if(GServer->ServerDebug)
            Log(MSG_DEBUG,"Executing Quest Trigger pakDoID");
        int success=thisclient->ExecuteQuestTrigger(hash_zone,true);
        if (success==QUEST_SUCCESS)
        {
	...
	pakInventoryCostume(thisclient);
...
bool CWorldServer::pakChangeStance( CPlayer* thisclient, CPacket* P )
{
	...
	if (stancenum == 2)
    {
		...
		if ((thisclient->Status->Stance==RUNNING || thisclient->Status->Stance==0x02) && map->allowpat == true)
		{// // Driving
            // Clean Buffs
            for(int i = 0; i < 15; i++)
            {
...
bool CWorldServer::pakChangeEquip( CPlayer* thisclient, CPacket* P )
{
    ...
    for(UINT i=0;i<15;i++)//Removes all positive buffs when equipment is changed
    {
        thisclient->MagicStatus[i].Duration = 0;
    }
...
bool CWorldServer::pakUserDied ( CPlayer* thisclient, CPacket* P )
{
    ...
    else
    {
        ...
        switch(map->id)
        {
            ...
            {
                thisrespawn->destMap=40;
                thisrespawn->dest.x=5184;
                thisrespawn->dest.y=5211;
            }
            break;
            case 41: //Oblivion Temple - B1
            case 42: //Oblivion Temple - B2
            case 43: //Oblivion Temple - B3
            case 44: //Oblivion Temple - B4
            {
                thisrespawn->destMap=41;
                thisrespawn->dest.x=5050;
                thisrespawn->dest.y=5250;
            }
            break;
            case 56: //Forgotten temple B1
            case 57: //Forgotten temple B2
            {
                thisrespawn->destMap=56;
            ...
            case 65: //Prison
            {
                thisrespawn->destMap=65;
                thisrespawn->dest.x=5485;
                thisrespawn->dest.y=5296;
            }
            break;
    ...
    {
        if(thisclient->Position->saved == 1)//Incase respawn id error
        {
            fPoint coord;
            coord.x = 5839;
            coord.y = 5332;
            MapList.Index[22]->TeleportPlayer( thisclient, coord, false );
        }else
        {
            fPoint coord;
            coord.x = 5654;
...
bool CWorldServer::pakStartSkill ( CPlayer* thisclient, CPacket* P )
{
	...
	//Prevents skills from being used if required skill aren't met
    if(thisclient->IsPlayer())
    {
        for(UINT i=0;i<3; i++)
        {
            if(thisskill->rskill[i] != 0)
            {
                ...
                if(rskill == 0xffff)
                {
                    Log(MSG_WARNING,"%s:: Necessary skill %u not found to use %u",thisclient->CharInfo->charname,thisskill->rskill[i],skillid);
                    return true;
                }
                else
                {
                    ...
                    {
                        Log(MSG_WARNING,"%s:: Necessary skill (%u) level incorrect (%u>%u) to use %u",thisclient->CharInfo->charname,thisskill->rskill[i],thisskill->lskill[i],thisclient->cskills[rskill].level,skillid);
                        return true;
                    }
                }
            }
    ...
    UINT needed_zulie=thisskill->costamount[0];
    if (thisskill->costtype[0] == 17)
    {
    ...
    {
        if (thisclient->CharInfo->Zulies < needed_zulie)
        {
            Log(MSG_HACK,"Player %s tried to pakStartSkill skill %u but didn't have enough Zulie %u<%u",thisclient->CharInfo->charname,skillid,thisclient->CharInfo->Zulies,needed_zulie);
            return true;
        }
    }

	UINT needed_mp2=(thisskill->costamount[1] - (thisskill->costamount[1] * thisclient->Stats->MPReduction / 100));
	UINT needed_zulie2=thisskill->costamount[1];
    if (thisskill->costtype[1] == 17)
    {
        if (thisclient->Stats->MP<needed_mp2)
        {
            Log(MSG_HACK,"Player %s tried to pakStartSkill skill %u but didn't have enough MP %u<%u",thisclient->CharInfo->charname,skillid,thisclient->Stats->MP,needed_mp);
            return true;
        }
    ...
    {
        if (thisclient->CharInfo->Zulies < needed_zulie2)
        {
            Log(MSG_HACK,"Player %s tried to pakStartSkill skill %u but didn't have enough Zulie %u<%u",thisclient->CharInfo->charname,skillid,thisclient->CharInfo->Zulies,needed_zulie);
            return true;
        }
...
bool CWorldServer::pakGiveQuest( CPlayer* thisclient, CPacket* P )
{
  ...
  if( thisclient->questdebug )
      Log(MSG_QUESTDEBUG, "pakGiveQuest:: Recieved 0x0730 packet hash: [%08x] Action: %i Slot: %i", hash, action,slot);

  if (action == 2)
  {
    if( thisclient->questdebug )
        Log(MSG_QUESTDEBUG, "pakGiveQuest:: %s:: %u ([%08x]), action %i slot %i delete quest",thisclient->CharInfo->charname,hash,hash,action,slot);
  ...
  if (action != 3)
  {
      Log(MSG_QUESTDEBUG, "pakGiveQuest:: action not 3, but %i Returning false",action);
      return false;
  }

  //what the heck is all this stuff that LMA put in here anyway?
  ...
  if (MapStackQuest.find(hash) != MapStackQuest.end())
  {
    ...
    for (int k=0;k<10;k++)
    {
        ...
        if (thisclient->arr_questid[k].die_time *3 >= mytime)
        {
    ...
    if(!is_ok)
    {
        ...
        if( thisclient->questdebug )
            Log(MSG_QUESTDEBUG, "pakGiveQuest:: LMA code sending 0x730 failure packet");
    ...
    //if(GServer->ServerDebug)
    //    Log(MSG_DEBUG,"LMA code found stackable quest. Return anyway!!");
    //return true;
    //PY this should stop quest item drops getting triggered twice.
  ...
  if( thisclient->questdebug )
        Log(MSG_QUESTDEBUG, "pakGiveQuest:: Executing Quest Trigger for hash %u", hash);
  int success = thisclient->ExecuteQuestTrigger(hash, false, slot);

  if( thisclient->questdebug )
        Log(MSG_QUESTDEBUG, "pakGiveQuest:: Sending 0x0730 packet. success %i. Slot %i ",success, slot);
  ...
  if( thisclient->questdebug )
        Log(MSG_QUESTDEBUG, "pakGiveQuest:: %s:: %u end",thisclient->CharInfo->charname,hash);

  if( thisclient->questdebug )
        Log(MSG_QUESTDEBUG, "pakGiveQuest:: %s:: %u ([%08x]), action %i slot %i result %i",thisclient->CharInfo->charname,hash,hash,action,slot,success);
  return true;
}
...
bool CWorldServer::pakUseItem ( CPlayer* thisclient, CPacket* P )
{
    ...
    switch(thisuse->usescript)
    {
       case 0: //  cherry berrys
        ...
        break;
        case 19://Akram Storage Coupon
        {
            Log(MSG_INFO,"%i:: Used a Akram Storage Coupon", thisclient->CharInfo->charname);
            flag=true;
        }
        break;
    }
...
bool CWorldServer::pakCraft( CPlayer* thisclient, CPacket* P )
{
    ...
    if(!painting)
    {
        ...
        if (failure>=100 && failure<200)
        {
            failure -= (failure*50/100);
        }
        else if (failure>=200 && failure<300)
        {
            failure -= (failure*50/100);
        }
        else if(failure>=300)
        ...
        failure-=(int) (thisclient->GetCon()/20);
        failure-=(int) (thisclient->GetSen()/20);
        failure-=(int) (thisclient->Stats->Level/20);
	...
	if (newslot !=0xffff)
    {
        ...
        if(!painting)
        {
			...
			{
			    crafting_exp = crafting_exp*(GServer->Config.doubleExpAmount*GServer->Config.EXP_RATE);
			}
			else
			{
                crafting_exp = crafting_exp*GServer->Config.EXP_RATE;
			}

            if(thisclient->Stats->Level < GServer->Config.MaxLevel) // level cap on crafting
...
bool CWorldServer::pakSellShop( CPlayer* thisclient, CPacket* P )
{
    ...
    switch(action)
    {
        case 0x01:
        {
            ...
            thisitem.count = otherclient->Shop->BuyingList[slot].count;
            RESETPACKET( pak, 0x7c6 );
            ADDWORD    ( pak, otherclient->clientid );
            ADDBYTE    ( pak, 0x07 );
            ADDBYTE    ( pak, 0x01 );
...
bool CWorldServer::pakModifiedItem( CPlayer* thisclient, CPacket* P )
{
    ...
    switch(action)
    {
        case 0x01://Gemming
        {
            ...
            {
                SendPM(thisclient,"You can't put a gem in this item!");
        ...
        case 0x03: //NPC Disassembly
        {
            BYTE src = GETBYTE((*P),3);
            char disassemblyzulie[200];
            int cost = ((GServer->STB_ITEM[thisclient->items[src].itemtype-1].rows[thisclient->items[src].itemnum][8]) * 10) + 20;

            if(thisclient->CharInfo->Zulies < cost)
            {
                Log(MSG_HACK,"%s: Disassembly zulie hack detected! cost: %i , Zulie: %i",thisclient->CharInfo->charname, cost, thisclient->CharInfo->Zulies);
                return true;
            }

            thisclient->CharInfo->Zulies = thisclient->CharInfo->Zulies - cost;

            sprintf (disassemblyzulie, "Your Zulie has decreased by %i", cost);
            SendSysMsg(thisclient, disassemblyzulie);

            Log(MSG_INFO,"%s NPC Disassembly, cost %i <= Zulie %i",thisclient->CharInfo->charname, cost,thisclient->CharInfo->Zulies);
...
bool CWorldServer::pakRepairItem( CPlayer* thisclient, CPacket* P,int packet_type)
{
    ...
    switch( packet_type)
    {
        case 1:
        {
            ...
            if (testnpc==NULL)
            {
                Log(MSG_HACK,"%s tried to repair with an unknown NPC! NPC id %i",thisclient->CharInfo->charname,npc_id);
                return true;
            }
			...
			liste_npc[6]=1129;	//Brock
			liste_npc[7]=1247;  //Kojo
			liste_npc[8]=2114;  //Ekblovo
			liste_npc[9]=2127;  //Mahyr
...
bool CWorldServer::pakRideRequest( CPlayer* thisclient, CPacket* P )
{
    ...
    switch(action)
    {
        ...
        case 0x02://Accept
        {
            ...
            if (thisclient->Status->Stance==DRIVING&&otherclient->Status->Stance==DRIVING)
            {
				Log(MSG_HACK,"%s tried to lift %s, but both are in carts",otherclient->CharInfo->charname, thisclient->CharInfo->charname);
                return true;
            }
...
{
    thisclient->lastPacketSent = clock();
}

bool CWorldServer::pakChangeCostume( CPlayer* thisclient, CPacket* P )
{
    if(thisclient->Shop->open)
        return true;
	WORD srcslot = GETWORD((*P), 0) + 140;
	WORD srcslot2 = GETWORD((*P), 0);
	WORD costume = GETWORD((*P), 0);
	WORD destslot = GETWORD((*P), 2);
	...
	thisclient->items[srcslot] = thisclient->items[destslot];
	thisclient->items[destslot] = tmpitm;
	thisclient->UpdateInventoryCostume((srcslot), destslot);

	if(thisclient->items[srcslot].itemnum == 0 && srcslot2 == 3)
	{
	    srcslot = 3;
	}
	if(thisclient->items[srcslot].itemnum == 0 && srcslot2 == 2)
	{
	    srcslot = 2;
	}
	if(thisclient->items[srcslot].itemnum == 0 && srcslot2 == 6)
	{
	    srcslot = 1;
	}
	if(thisclient->items[srcslot].itemnum == 0 && srcslot2 == 4)
	{
	    srcslot = 5;
	}
	if(thisclient->items[srcslot].itemnum == 0 && srcslot2 == 5)
	{
	    srcslot = 6;
	}
	if(thisclient->items[srcslot].itemnum == 0 && srcslot2 == 7)
	{
	    srcslot = 4;
	}
