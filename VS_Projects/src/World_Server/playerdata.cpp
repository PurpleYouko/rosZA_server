...
bool CPlayer::loaddata( )
{
    ...
    bonusddrop = atoi(row[45]);     // hmm.... Medals get saved when you log out??
    ...
    wait_validation=0;
    wait_validation_ddrop = 0;      //Set to default 0 whatever bonusddrop loads in as. Again, utterly pointless variable
    ...
    if(bonusddrop <= 0) bonusddrop = 0; //value for number of extra drop chances
    ...
    if(bonusddrop > 0 && (etime >= timerddrop)) {bonusddrop = 0; timerddrop = 0;} //timer elapsed
    ...
    /* WTF?? this is exactly the same code as the 3 rows above. Why is it all checked twice?
    if(bonusddrop > 0 && (etime >= timerddrop))
    {
      bonusddrop = 0;
      timerddrop = 0;
    }
...
void CPlayer::savedata( )
{
    ...
    if(Session->userid!=0)
    {
	   ...
	   if(Stats->MP < 0)
        ...
        GServer->DB->QExecute("UPDATE accounts SET ExpansionTime = %i WHERE username = '%s'",timerexapnsion,Session->username);
...
//Auto RP/DP - Players receive DP as they play
void CPlayer::DP( )
{
    lastDpTime = clock();
    int DP = 0;
    ...
    clock_t mayGive;
    MYSQL_ROW row;
    MYSQL_RES *result = GServer->DB->QStore("SELECT nb_donation, counter FROM accounts WHERE id = %i", Session->userid);
    row = mysql_fetch_row(result);
    ...
    mayGive = clock() - lastPacketSent;

    if(Counter >= GServer->Config.TimerDP && mayGive <= 600000)
    {
        Log(MSG_INFO, "Player ID %i received DP. Amount of DP before top-up = %i.", Session->userid, DP);
        DP += GServer->Config.AmountDP;
        GServer->DB->QExecute("UPDATE accounts SET nb_donation = %i, Counter = '0' WHERE id = %i", DP, Session->userid);
    }
    else if(mayGive <= 600000)
    {
        Counter += GServer->Config.EXP_RATE;
        GServer->DB->QExecute("UPDATE accounts SET counter = %i WHERE id = %i", Counter, Session->userid);
    }
}
...
void CPlayer::clearquest(CPlayer* thisclient)
{
    memset(&quest, 0, sizeof(SQuestData));
}
