...
bool CPlayer::CheckPlayerLevelUP( )
{
	...
	while (CharInfo->Exp >= GetLevelEXP() && Stats->Level < GServer->Config.MaxLevel)
    {
        //LMA: TEST:
        //Log(MSG_INFO,"%s should level up (%I64i >= %u), level: %u",CharInfo->charname,CharInfo->Exp,GetLevelEXP(),Stats->Level);
...
bool CPlayer::GetPlayerInfo( )
{
	...
	ADDBYTE( pak, 0 );
	client->SendPacket(&pak);
    sprintf(text,"Accuracy: %i | Dodge: %i",Stats->Accuracy,Stats->Dodge );
	RESETPACKET( pak, 0x0784 );
	ADDSTRING( pak, "[GM]PlayerInfo" );
	ADDBYTE( pak, 0 );
	ADDSTRING( pak, text );
...
bool CPlayer::PlayerHeal()
{
	...
	if( UsedItem->usevalue!=0 && transtime >= 3*CLOCKS_PER_SEC )
    {
...
{
    //return Attr->Int;
    return (Attr->Int + Attr->Eint + (int)Attr->MEint); //Tomiz : We need to take int from cloth,gem,ect into consideration(ExtraInt/Eint)
}

// return Sensibility
unsigned int CPlayer::GetSen( )
{
    return (Attr->Sen + Attr->Esen + (int)Attr->MEsen); //Tomiz : We need to take sen from cloth,gem,ect into consideration(ExtraSen/Esen)
}

// return Concentration
unsigned int CPlayer::GetCon( )
{
    return (Attr->Con + Attr->Econ + (int)Attr->MEcon); //Tomiz : We need to take con from cloth,gem,ect into consideration(ExtraCon/Econ)
}
...
void CPlayer::UpdateInventoryCostume( unsigned int slot1, unsigned int slot2, bool save)
{
    if(slot1==0xffff && slot2==0xffff) return;
    BEGINPACKET( pak, 0x726 );
    if(slot1!=0xffff && slot2!=0xffff) {ADDBYTE( pak, 2 );}
    else {ADDBYTE( pak, 1 );}
    if(slot1!=0xffff)
    {
        ADDBYTE    ( pak, (slot1-140));
        ADDDWORD   ( pak, GServer->BuildItemHead( items[slot1] ) );
        ADDDWORD   ( pak, GServer->BuildItemData( items[slot1] ) );
        ADDDWORD( pak, 0x00000000 );
        ADDWORD ( pak, 0x0000 );
...
int CPlayer::ExecuteQuestTrigger(dword hash,bool send_packet, UINT index)
{
    if( GServer->questdebug )
        Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger:: BEGIN %u for %s, index %u",hash,CharInfo->charname,index);
    ...
    for(unsigned j=index; j < GServer->TriggerList.size(); j++)
    {
      if (GServer->TriggerList.at(j)->TriggerHash == hash)
      {
        ...
        if(j+1<GServer->TriggerList.size())
        {
            ...
            if( GServer->questdebug )
                Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger::  %u for %s:: next trigger %u, %s FORCED",hash,CharInfo->charname,nexttrigger->TriggerHash,nexttrigger->TriggerName);
        }
        else
        {
            if( GServer->questdebug )
                Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger::  %u for %s:: next trigger is NULL!",hash,CharInfo->charname);
        }
    ...
    {
        if( GServer->questdebug )
            Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger::  questid %u not found",hash);
    ...
    for (dword i = 0; i < trigger->ConditionCount; i++)
    {
      ...
      if (success == QUEST_FAILURE)
      {
        ...
        {
            if( GServer->questdebug )
        Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger:: No checknext (FAILURE)");
            return success;
        }
        else
        {
            if( GServer->questdebug )
                Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger:: checknext because FAILURE, will do %u",nexttrigger->TriggerHash);

            if(nexttrigger==NULL)
            {
                if( GServer->questdebug )
                    Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger:: Next trigger but NULL! %u",hash);
                return QUEST_SUCCESS;
            }
            if( GServer->questdebug )
                Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger:: Executing Quest Trigger for NextTrigger from Playerfunction");
      ...
      if( GServer->questdebug )
        Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger:: Quest cdt %i / %i success",i+1,trigger->ConditionCount);
    }
    ...
    for (dword i = 0; i < trigger->ActionCount; i++)
    {
      ...
      {
          if( GServer->questdebug )
        Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger:: unknown Action command %i", command);
          continue;
      }
      ...
      if( GServer->questdebug )
        Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger:: Reward %03u returned %d, %i / %i", command, (*GServer->qstRewdFunc[command])(GServer, this, trigger->Actions[i]->data),i+1,trigger->ActionCount);
    }
    ...
    if( GServer->questdebug )
        Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger::  END %u for %s",hash,CharInfo->charname);
...
void CPlayer::SendQuestUpdate()
{
    //if( GServer->questdebug )
        Log(MSG_QUESTDEBUG, "SendQuestUpdate:: Sending Quest Update for player");
    ...
    for( unsigned i = 0; i < 10; i++ )
    {
        ...
        for(dword j = 0; j < 10; j++) ADDWORD( pak, quest.quests[i].Variables[j]);
        for(dword j = 0; j < 4; j++) ADDBYTE( pak, quest.quests[i].Switches[j]);
        for(dword j = 0; j < 6; j++)
        {
...
bool CPlayer::CheckStats(int slot, int dest_slot)
{
    ...
    for(int k=0;k<2;k++)
    {
        ...
        switch(cdt)
        {
            ...
            case sStrength:
            {
                player_value=(Attr->Str + Attr->Estr);
            }
            break;
            case sDexterity:
            {
                player_value=(Attr->Dex + Attr->Edex);
            }
            break;
            case sIntelligence:
            {
                player_value=(Attr->Int + Attr->Eint);
            }
            break;
            case sConcentration:
            {
                player_value=(Attr->Con + Attr->Econ);
            }
            break;
            case sCharm:
            {
                player_value=(Attr->Cha + Attr->Echa);
            }
            break;
            case sSensibility:
            {
                player_value=(Attr->Sen + Attr->Esen);
            }
            break;
            default:
            {
