    ...
    */
    ...
    switch(type)
    {
        ...
        case 313://Magic Item [scrolls/summons/fireworks/etc]
        {
            ...
            if( (useitem->itemnum>385 && useitem->itemnum<390) ||
                (useitem->itemnum>400 && useitem->itemnum<440) ||
                ...
                (useitem->itemnum==1055) )
            {
                ...
                useitem->usescript = 5;
                                Log(MSG_INFO,"itemnum: %i usevalue: %i",useitem->itemnum, useitem->usevalue);
                useitem->usetype = UseList.Index[useitem->itemnum]->useeffect[1];
...
//                else if(useitem->itemnum==609){useitem->usevalue = 2047;} // Event Metal Moldie
                ...
                else if(useitem->itemnum==386){useitem->usevalue = 336;} // Summons Wolf
                else if(useitem->itemnum==387){useitem->usevalue = 376;} // Summons Mammoth
                else if(useitem->itemnum==388){useitem->usevalue = 373;} // Summons Dreadnought
                else if(useitem->itemnum==389){useitem->usevalue = 976;} // Summons Lunaris Bear
...
//                else if(useitem->itemnum==944){useitem->usevalue = 172;} // Arua's Blessing
        ...
        case 321://Time Coupon - New Way to check TC
        {
              ...
              switch (UseList.Index[useitem->itemnum]->useeffect[0])
              {
                    ...
                    {
                        if(!thisclient->storageExpanded)
                        {
                            Log(MSG_INFO,"%s used Storage Expansion Coupon!",thisclient->CharInfo->charname);
                            useitem->usescript = 18;
                            useitem->usetype =0;
                            thisclient->storageExpanded=true;

                            //7 day, will "resist" to logout
                            thisclient->timerexapnsion=time(NULL)+604800;
                            ...
                            //ADDBYTE( pak, timeinfo->tm_min+60); //minutes?
                        ...
                        {
                            BEGINPACKET( pak, 0x702 );
                            ADDSTRING( pak, "You already used a Storage Expansion Coupon!" );
                            ADDBYTE( pak, 0 );
                            thisclient->client->SendPacket(&pak);
                            return NULL;
                        }
                    ...
                    break;
                    case 134://Medal of Fortune or Sigil of Wealth etc.. Set bonusddrop to quality - 1
                    {
                        if (thisclient->timerddrop == 0)
                        {
                            thisclient->bonusddrop = UseList.Index[useitem->itemnum]->quality - 1;
                            thisclient->wait_validation_ddrop = UseList.Index[useitem->itemnum]->quality - 1; //quality
...
void CWorldServer::TakeItemMallList(CPlayer* thisclient,int qty,int slot)
{
    ...
    if(newitem.itemtype > 9 && newitem.itemtype < 14) //stackable
    {
        int count = qty;
        if( count>thisclient->itemmallitems[slot].count )
            count = thisclient->itemmallitems[slot].count;
        newitem.count = count;
        thisclient->itemmallitems[slot].count -= count; //reduce count in storage
    ...
    {
        ClearItem(thisclient->itemmallitems[slot]); //empty storage slot. Not stackable
    }
