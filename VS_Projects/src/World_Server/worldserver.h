...
//LMA: Grids
#define NB_MAPS 122        //Nb maps (120 maps, including bad and non existant).
...
class CWorldServer : public CServerSocket
{
    public:
        ...
        void pakInventoryCostume( CPlayer *thisclient );
...
//        bool LearnSkill( CPlayer* thisclient, UINT skill );
    	...
    	bool LoadZoneData( );
		//How about we just DON't load these stupid grids that crash everything for no specific gain
        ...
        fpAiCond aiCondFunc[41];
        fpAiAct aiActFunc[43];
		...
		vector<CMDrops*>		ShortList;
		//vector<CItemType>		Prob;
		//vector<UINT>			MDropItemnumber;
		//vector<UINT>			MDropItemType;
		//vector<UINT>			MDropProb;
		//vector<UINT>			MDropAlt;
       ...
       // geo edit for disassemble // 22 oct 07
		...
		//PY
		int TDSpawnCount;               //The number of monsters remaining to be spawned in a current Tower Defense wave
		int TDMonCount;                 //Number of monsters currently alive in the current TD wave
		UINT TDSpawnType;               //Monster ID for the current TD wave
		UINT TDWaves;                   //Number of waves remaining in the current TD game
		UINT TDMap;                     //Map ID for the current TD game
		UINT TDStance;                  //Current stance for TD mobs
		UINT TDLives;                   //Life counter
		clock_t TDSpawnTimer;           //need a timer to determine how fast to spawn TD mobs
		UINT TDSpawnRate;               //The rate at which the TD mobs spawn
		bool CurrentTD;                 //Is there currently a TD game going on?
