...
void Log( enum msg_type flag, const char *Format, ... )
{
    ...
    time(&rtime);
	char timestamp [ 26 ];
	ctime_s ( timestamp, sizeof ( timestamp ), &rtime );
	...
	if(PRINT_LOG)
	{
        switch (flag) {
                case MSG_NONE: // direct printf replacement
								//changed logging to show message type in logfile - T3
                        textcolor(WHITE);
                        vprintf( Format, ap );
                        break;
                case MSG_STATUS:
                        textcolor(GREEN);
                        sprintf_s(msgtype,"%s","[STATUS]: ");
                        printf("%s",msgtype);
                        break;
                case MSG_SQL:
                        textcolor(CYAN);
                        sprintf_s(msgtype,"%s","[SQL]: ");
                        printf("%s",msgtype);
                        break;
                case MSG_INFO:
                        textcolor(LIGHTGREEN);
                        sprintf_s(msgtype,"%s","[INFO]: ");
                        printf("%s",msgtype);
                        break;
                case MSG_NOTICE:
                        textcolor(LIGHTCYAN);
                        sprintf_s(msgtype,"%s","[NOTICE]: ");
                        printf("%s",msgtype);
                        break;
                case MSG_WARNING:
                        textcolor(YELLOW);
                        sprintf_s(msgtype,"%s","[WARNING]: ");
                        printf("%s",msgtype);
                        break;
                case MSG_DEBUG:
                        textcolor(LIGHTBLUE);
                        sprintf_s(msgtype,"%s","[DEBUG]: ");
                        printf("%s",msgtype);
                        break;
                case MSG_ERROR:
                        textcolor(RED);
                        sprintf_s(msgtype,"%s","[ERROR]: ");
                        printf("%s",msgtype);
                        break;
                case MSG_FATALERROR:
                        textcolor(LIGHTRED);
                        sprintf_s(msgtype,"%s","[FATAL ERROR]: ");
                        printf("%s",msgtype);
                        break;
                case MSG_HACK:
                        textcolor(LIGHTRED);
                        sprintf_s(msgtype,"%s","[HACK]: ");
                        printf("%s",msgtype);
                        break;
                case MSG_LOAD:
                        textcolor(BROWN);
                        sprintf_s(msgtype,"%s","[LOADING]: ");
                        printf("%s",msgtype);
                        break;
                case MSG_GMACTION:
                        textcolor(MAGENTA);
                        sprintf_s(msgtype,"%s","[GM ACTION]: ");
                        printf("%s",msgtype);
                        break;
                case MSG_QUESTDEBUG:
                        textcolor(LIGHTBLUE);
                        sprintf_s(msgtype,"%s","[QUEST DEBUG]: ");
                        printf("%s",msgtype);
                        break;
        case MSG_START:
            textcolor(MAGENTA);
...
void LogHandleSp(int type)
{
    ...
    {
        if(fhSp1==NULL)
            fopen_s ( &fhSp1, LOG_DIRECTORY "export.log", "w+" );

        if(fhSpDebug==NULL)
            fopen_s ( &fhSpDebug, LOG_DIRECTORY "debug.log", "a+" );
    }
    else
    {
        if(fhSp1!=NULL)
