...
#pragma warning(disable:4129)		// Disable Compiler Warning C4129 (Unrecognized Character escape sequence (log.cpp, this is actually misinterpret.) ->FK.
...
struct CROSEServerConfig
{
    ...
    int Command_DelTicket; //Delete ticket
    ...
    int Command_ReadTick;  //Read Ticket
    ...
    // Warning system
    int Command_Warning;
    int Command_SeeWarning;
    int Command_DelWarning;
    int warningSysActive;
    int ban_time1;
    int ban_time2;
    int ban_timer;
    clock_t lastWCountRed;        //Last Ban warning counter reduction

	//Lotto Zulie/DP
    int LottoType;
    int LottoActive;
    long long int ZulieCost;
    int Command_Lotto;
    long long int ZuliePrize;
    long long int ZulieLimit;
    clock_t lastLastLottoAnn;

    //Advertise system
    int AdvertiseActive;
    int numAds;
    clock_t AdvertiseTime;
    int AdvertiseClock;
    string Adverd[200];
    int lastAd;
};
...
void    BanTracker(); // Tracks how much is left till a players temp ban is removed
...
void GivePrizes();
void GiveZuliePrize(char name[20], int numbers);
void DeleteLottoRecords();
void swap(int& a,int& b, bool& swapped);
void change(int& c, bool& mayCange);
void LottoAnn();
