...
bool CCharServer::pakClanManager ( CCharClient* thisclient, CPacket* P )
{
    ...
    switch (action)
    {
        ...
        case 0x02://invite clan
        {
            ...
            if(_stricmp(nick,thisclient->charname)==0)
            {
                delete []nick;
                return true;
            }
        ...
        case 0x03://Kick from clan
        {
           ...
           if(thisclan==NULL)
           {
            ...
            for(UINT i=0;i<thisclan->ClanMembers.size( );i++)
            {
                ...
                if(_stricmp(ClanMember->name,nick)==0)
                {
           ...
           if( otherclient!=NULL )
           {
            ...
            if(!DB->QExecute("UPDATE characters SET clanid=0, clan_rank=1 WHERE char_name='%s'",nick))
            {
                delete []nick;
                return false;
            }
        ...
        case 0x04://up rank
        {
            ...
            CClans*	Clan = (CClans*) GetClanByID( thisclient->clanid );
            result = DB->QStore("SELECT id,char_name,clan_rank FROM characters WHERE clanid=%i order by clan_rank",Clan->id);
            if(result==NULL) return false;
            while(row = mysql_fetch_row(result))
            {
                CClanMembers* thismember = Clan->ClanMembers.at( i );
                assert(thismember);
                thismember->id = atoi(row[0]);
                strcpy_s(thismember->name,row[1]);
                thismember->clan_rank = atoi(row[2]);
                Log(MSG_INFO,"Updated clan info for %s , because of promotion!",thismember->name);
                i++;
            }
            DB->QFree( );
        	delete []nick;
        ...
        case 0x05://down rank
        {
            ...
            while(row = mysql_fetch_row(result))
            {
                ...
                strcpy_s(thismember->name,row[1]);
                thismember->clan_rank = atoi(row[2]);
                Log(MSG_INFO,"Clan info updated for %s , because of demotion in clan",thismember->name);
                i++;
            }
            DB->QFree( );
        ...
        case 0x07://Leave Clan
        {
           ...
           SendToClanMembers(thisclient->clanid,&pak);
            ...
            for(UINT i=0;i<thisclan->ClanMembers.size();i++)
            {
                ...
                if(_stricmp(ClanMember->name,thisclient->charname)==0)
                {
                    thisclan->ClanMembers.erase( thisclan->ClanMembers.begin()+i );
                    delete ClanMember;
                    break;
        ...
        case 0x09://Give Master
        {
            ...
            while(row = mysql_fetch_row(result))
            {
                ...
                strcpy_s(thismember->name, row[1]);
                thismember->clan_rank = atoi(row[2]);
                Log(MSG_HACK,"Clan info update for %s , because of master change!",thismember->name);
                i++;
            }
            DB->QFree( );
            delete []nick;
        ...
        case 0x0c://invitation accepted
        {
            ...
            if(otherclient!=NULL)
            {
                ...
                strcpy_s(newmember->name,thisclient->charname);
        ...
        case 0xfa://message from worldserver to load the new clan information
        {
           ...
           if(otherclient==NULL)
        	...
        	strncpy_s(newclan->name,row[3],16);
        	newclan->cp = atoi(row[4]);
        	newclan->grade = atoi(row[5]);
            strncpy_s(newclan->slogan,row[6],29);
            strcpy_s(newclan->news,row[7]);
            ...
            strcpy_s(newmember->name,otherclient->charname);
...
bool CCharServer::pakUploadCM ( CCharClient* thisclient, CPacket* P )
{
	...
	FILE* fh = NULL;
	fopen_s (&fh, "clanmark/clanmark.cnt", "r+" );
    if(fh==NULL)
    {
    ...
    thisclan->back = 0;
    char filename[30];
    sprintf_s( filename, "clanmark/%u.cm", cmid );

    //LMA: switched to binary mode
    fopen_s(&fh, filename, "w+b" );
    if(fh==NULL)
    {
