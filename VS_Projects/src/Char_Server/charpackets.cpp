...
bool CCharServer::pakDoIdentify( CCharClient* thisclient, CPacket* P )
{
    ...
    else
    {
		...
		strncpy_s(thisclient->username, row[0],16);
...
bool CCharServer::pakGetCharacters( CCharClient* thisclient, CPacket* P )
{
	...
	while (row = mysql_fetch_row(result))
    {
		...
		strcpy_s( chars[charnum].char_name , row[0] );
...
//              //          0        1
	...
	while(row = mysql_fetch_row( result ))
	{
        ...
        strcpy_s(newfriend->name, row[1]); //friend name
        thisclient->FriendList.push_back( newfriend );
    //
    // Toxins friend fix - http://forum.dev-osrose.com/viewtopic.php?f=21&t=4607
    //
        ADDDWORD    (pak, newfriend->id);// friend id
        //ADDWORD    (pak, 0x0000 );
...
bool CCharServer::pakDeleteChar( CCharClient* thisclient, CPacket* P )
{
	...
	if (_stricmp(row[0], thisclient->username)!=0)
	{
