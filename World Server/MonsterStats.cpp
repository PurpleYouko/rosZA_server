...
unsigned int CMonster::GetAttackPower()
{
	...
	if(IsSummon())//Tomiz : Check and Add The Owner AP To Base Summon Value(STB)
    {
            ...
            if(ownerclient!=NULL)
            {
                ...
                scale = lvlp*0.003;
                attack += (long int)floor(ownerclient->Stats->Attack_Power * scale);
...
unsigned int CMonster::GetDefense()
{
    ...
    if(IsSummon())//Tomiz : Check and Add The Owner DEFF To Base Summon Value(STB)
    {
        ...
        {
            UINT lvlp = thisnpc->level;
            float scale = lvlp*0.003;
            defense += (long int)floor((ownerclient->Stats->Defense * scale));
...
unsigned int CMonster::GetDodge()
{
    ...
    if(IsSummon())//Tomiz : Check and Add The Owner DODGE To Base Summon Value(STB)
    {
        ...
        if(ownerclient!=NULL)
        {
            UINT lvlp = thisnpc->level;
            float scale = lvlp*0.003;
            dodge += (long int)floor((ownerclient->Stats->Dodge * scale));
...
// return Accuracy
unsigned int CMonster::GetAccuracy()
{
    UINT hitrate = 0;
    ...
    if(IsSummon())//Tomiz : Check and Add The Owner ACC To Base Summon Value(STB)
    {
        ...
        if(ownerclient!=NULL)
        {
            UINT lvlp = thisnpc->level;
            float scale = lvlp*0.003;
            hitrate += (long int)floor((ownerclient->Stats->Accuracy * scale));
            Log(MSG_INFO,"SUMMON %i Got %i Acc from is owner, lvlp: %i",thisnpc->id,(hitrate - thisnpc->hitrate),lvlp);
        }
    }

    if(Status->Accuracy_up!=0xff)
    {
        hitrate += MagicStatus[Status->Accuracy_up].Value;
    }
    if(Status->Accuracy_down!=0xff)
    {
        hitrate -= MagicStatus[Status->Accuracy_down].Value;
    }
...
unsigned int CMonster::GetMagicDefense()
{
    ...
    if(IsSummon())//Tomiz : Check and Add The Owner MDEFF To Base Summon Value(STB)
    {
        ...
        if(ownerclient!=NULL)
        {
            UINT lvlp = thisnpc->level;
            float scale = lvlp*0.003;
            mdef += (long int)floor((ownerclient->Stats->Magic_Defense * scale));
...
unsigned int CMonster::GetGreyDrops( )
{
    return 0;
}
...
bool CMonster::SetStats(bool all)
{
    ...
    if (all||Stats->xprate == 0)
    {
        Stats->xprate = GetXPRate( );
    }

    if (all||Stats->itemdroprate == 0)
    {
        Stats->itemdroprate = GetItemDropRate( );
    }
    if (all||Stats->zulydroprate == 0)
    {
        Stats->zulydroprate = GetZulyDropRate( );
    }

    if (all||Stats->greydrop == 0)
    {
        Stats->greydrop = GetGreyDrops( );
    }
