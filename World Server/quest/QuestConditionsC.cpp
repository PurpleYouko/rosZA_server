...
QUESTCONDC(010){
	...
	if(thischance >= data->btLowPcnt and thischance <= data->btHighPcnt)
	{
	    if(GServer->ServerDebug)
            Log(MSG_DEBUG," (Character) Percent chance SUCCESS %i < %i", thischance,data->btHighPcnt);
	    //if( client->questdebug )
        //    server->SendPM(client, "Percent chance SUCCESS %i < %i", thischance,data->btHighPcnt);
	    return QUEST_SUCCESS;
	}
	if(GServer->ServerDebug)
        Log(MSG_DEBUG,"(Character) Percent chance FAILURE %i > %i", thischance,data->btHighPcnt);
	//if( client->questdebug )
    //    server->SendPM(client, "Percent chance FAIL %i > %i", thischance,data->btHighPcnt);
